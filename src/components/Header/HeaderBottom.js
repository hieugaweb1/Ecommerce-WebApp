...
class HeaderBottom extends Component {
  ...
  async componentDidMount() {
    ...
    const res = await callApi(`producers`, 'GET', null);
    let temp = []
    
    this.setState({ brands: res.data.results })
  }
                ...
                <nav className="navbar navbar-expand-sm navbar-light bg-light">
                  ...
                  <div className="collapse navbar-collapse" id="collapsibleNavId">
                    <ul className="navbar-nav mr-auto mt-2 mt-lg-0">
                      ...
                      <li className="nav-item dropdown">
                        <Link className="nav-link dropdown-toggle" to="#" id="dropdownId" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">BRANDS</Link>
                        <div className="dropdown-menu" aria-labelledby="dropdownId">
                        {this.state.brands.map((brand, index) => 
                           <a key={index} className="dropdown-item" href={`/products/brand/${brand.id}`} onClick={this.handleSelectBrand}>{brand.name}</a>
                        )}
                        </div>
                      </li>
