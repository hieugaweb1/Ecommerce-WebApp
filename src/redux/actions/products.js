...
export const actSearchProductsRequest = (q) => {
    return async dispatch => {
        dispatch(actShowLoading());
        const res = await callApi(`products?keyword=${q}`, 'GET', null);
        if (res && res.status === 200) { 
...
export const actFetchProductsLaptopRequest = (q) => {
    ...
    const limit = 8;
    return async dispatch => {
        const res = await callApi(`categories/8/products?limit=${limit}&offset=${offset}&orderBy=-createdAt`, 'GET', null);
        if (res && res.status === 200) {
...
export const actFetchProductsOfficeRequest = (q) => {
    ...
    const limit = 10;
    return async dispatch => {
        const res = await callApi(`categories/6/products?limit=${limit}&offset=${offset}&orderBy=-createdAt`, 'GET', null);
        if (res && res.status === 200) {
